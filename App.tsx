...
import { PaymentMethod, DeviceType, OutPartyEntry, MainEntry, HistoryRecord } from './types.ts';
import { fetchExchangeRates } from './services/geminiService.ts';
...
const App: React.FC = () => {
  ...
  const [gunInstance] = useState(() => Gun({ peers: MESH_RELAYS, localStorage: true }));
  ...
  // --- Peer Sync ---
  useEffect(() => {
    if (!syncId || !gunInstance) return;
    const db = gunInstance.get('shivas_v11').get(syncId);
    ...
    db.get('balance').on((v) => {
      if (v !== undefined && v !== null) setOpeningBalance(parseFloat(v as string));
    });
  ...
  const addOutParty = (method: PaymentMethod, amount: number) => {
    ...
    gunInstance.get('shivas_v11').get(syncId).get('outParty').get(id).put(JSON.stringify(entry));
  };

  const removeOutParty = (id: string) => {
    if (!isEditor || !syncId) return;
    gunInstance.get('shivas_v11').get(syncId).get('outParty').get(id).put(null);
  };
  ...
  const addMainEntry = (roomNo: string, description: string, method: PaymentMethod, cashIn: number, cashOut: number) => {
    ...
    gunInstance.get('shivas_v11').get(syncId).get('main').get(id).put(JSON.stringify(entry));
  };

  const removeMainEntry = (id: string) => {
    if (!isEditor || !syncId) return;
    gunInstance.get('shivas_v11').get(syncId).get('main').get(id).put(null);
  };
    ...
    <div className="min-h-screen pb-48">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-10">
          ...
          <div className="flex items-center gap-6">
            <div className="bg-slate-50 border-4 border-slate-100 px-8 py-4 rounded-[2.5rem] flex flex-col items-center min-w-[160px]">
              ...
              <span className="text-3xl font-black text-slate-950">Rs. {rates.usd}</span>
            </div>
            <div className="bg-slate-50 border-4 border-slate-100 px-8 py-4 rounded-[2.5rem] flex flex-col items-center min-w-[160px]">
              <span className="text-xs font-black text-slate-400 uppercase tracking-widest">🇪🇺 EUR</span>
              <span className="text-3xl font-black text-slate-950">Rs. {rates.eur}</span>
            </div>
            {isEditor && (
              ...
              <section className="bg-white rounded-[4rem] shadow-2xl border-4 border-slate-900 overflow-hidden">
                <div className="bg-blue-800 p-12 flex flex-col md:flex-row justify-between items-center gap-10 text-white">
                  ...
                  <div className="flex flex-wrap gap-6">
                    <StatBox label="CASH" value={totals.opCash} color="bg-blue-900" />
                    <StatBox label="CARD" value={totals.opCard} color="bg-amber-600" />
                    <StatBox label="PAYPAL" value={totals.opPaypal} color="bg-purple-700" />
                  </div>
                </div>
                {isEditor && <div className="p-12 bg-blue-50/50"><OutEntryForm onAdd={addOutParty} /></div>}
                    ...
                    <tbody className="divide-y-4 divide-slate-50">
                      {outPartyEntries.map(e => (
                        <tr key={e.id}>
                          ...
                          <td className="px-14 py-10"><PaymentTag method={e.method} /></td>
                ...
                <div className="bg-slate-900 p-16 text-white">
                  <div className="flex flex-col lg:flex-row justify-between items-center gap-14">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-10 w-full lg:w-auto">
                      <SummaryBlock label="CASH IN" value={totals.totalCashIn} color="text-blue-400" />
                      <SummaryBlock label="CASH OUT" value={totals.totalCashOut} color="text-red-400" />
                      <SummaryBlock label="NET BAL" value={totals.balance} color="text-green-400" highlight />
                    </div>
                  </div>
                </div>
                {isEditor && <div className="p-14 bg-slate-50"><MainEntryForm onAdd={addMainEntry} /></div>}
                <div className="overflow-x-auto">
                    ...
                    <tbody className="divide-y-4 divide-slate-100">
                      {openingBalance !== 0 && (
                        <tr className="bg-emerald-50">
                          ...
                          <td className="px-16 py-14 text-center"><PaymentTag method={PaymentMethod.CASH} /></td>
                      ...
                      {mainEntries.map(e => (
                        <tr key={e.id}>
                          ...
                          <td className="px-16 py-14 text-center"><PaymentTag method={e.method} /></td>
      ...
      {showQr && (
        <div className="fixed inset-0 bg-slate-950/95 z-[200] flex items-center justify-center p-8">
          <div className="bg-white p-24 rounded-[7rem] text-center space-y-16 max-w-2xl w-full shadow-2xl">
            <h2 className="text-5xl font-black uppercase">Pair Mobile Device</h2>
...
const SummaryBlock = ({ label, value, color, highlight = false }: { label: string, value: number, color: string, highlight?: boolean }) => (
...
const PaymentTag = ({ method }: { method: PaymentMethod }) => {
  const styles = {
...
const OutEntryForm = ({ onAdd }: { onAdd: (m: PaymentMethod, a: number) => void }) => {
  ...
  return (
    <form className="flex flex-wrap items-end gap-10" onSubmit={e => { e.preventDefault(); onAdd(method, parseFloat(amount)); setAmount(''); }}>
      ...
      <button type="submit" className="px-20 py-8 bg-blue-700 text-white font-black rounded-[2.5rem] uppercase text-sm border-b-8 border-blue-950">Add Entry</button>
    </form>
  );
};
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/98 z-[200] flex items-center justify-center p-8">
      <div className="bg-white p-24 md:p-40 rounded-[7rem] shadow-2xl max-w-4xl w-full text-center space-y-20">
        <h2 className="text-8xl font-black uppercase tracking-tighter">Vault Pair</h2>
        <div className="space-y-12">
          ...
          <button onClick={() => id && onConnect(id.trim())} className="w-full p-20 bg-blue-700 text-white font-black rounded-[5rem] text-5xl uppercase tracking-[0.5em] border-b-[20px] border-blue-950">Initialize</button>
        </div>
        <p className="text-slate-400 font-black uppercase text-xl tracking-widest">Global Financial Mesh Protocol Active</p>
      </div>
    </div>
  );
};
